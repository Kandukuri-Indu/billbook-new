...
const  EditQuotations = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/addQuotation/quotations")
      .then((response) => {
  ...
  const [formData, setFormData] = useState({
    quotationName: "Quotation Invoice",
  ...
  const handlePaymentTermsChange = (terms) => {
    ...
    const totalDays = Math.ceil(
      (formData.dueDate.getTime() - formData.quotationDate.getTime()) /
        (24 * 60 * 60 * 1000) +
        1
    );
    ...
    setFormData((prevData) => ({
      ...
      dueDate:
        formData.quotationDate.getTime() + (totalDays - 1) * 24 * 60 * 60 * 1000, // Calculate dueDate based on totalDays
    }));
  };
  ...
  //     // Calculate the difference in days between invoiceDate and dueDate
  //     const timeDiff = Math.abs(
  //       date.getTime() - formData.quotationDate.getTime()
  //     );
  //     const diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24));
  ...
  //     // Update Due Date based on Payment Terms
  //     if (fieldName === "paymentTerms" && formData.quotationDate) {
  //       const dueDate = new Date(formData.quotationDate);
  //       dueDate.setDate(dueDate.getDate() + parseInt(date, 10));
  //       setFormData((prevData) => ({
  //         ...prevData,
  //         dueDate,
  ...
  //     // Update Payment Terms based on Due Date
  //     if (fieldName === "dueDate" && formData.quotationDate) {
  //       const differenceInDays = Math.floor(
  //         (date - formData.quotationDate) / (24 * 60 * 60 * 1000)
  //       );
  //       setFormData((prevData) => ({
  //         ...prevData,
  //         paymentTerms: differenceInDays.toString(),
    ...
    const fetchQuotationDetails = async (id) => {
      try {
        ...
        if (response.ok) {
          ...
          console.log("GrandTotalForAll", quotationDetails.grandTotal);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/api/addQuotation/update-quotation/${id}`,
        { formData }
      );
  ...
  const getNextQuotationNumber = () => {
    if (datasource.length === 0) {
      ...
      const maxQuotationNumber = Math.max(
        ...datasource.map((quotation) => quotation.quotationNumber)
      );
      return maxQuotationNumber + 1; 
    }
  };
                        ...
                        <div className="col-lg-4 col-md-6 col-sm-12">
                          <div className="form-group">
                            ...
                            <input
                              ...
                              value={
                                formData.quotationNumber || getNextQuotationNumber()
                              }
                              disabled
                              // onChange={(e) =>
                              //   handleInputChange(
