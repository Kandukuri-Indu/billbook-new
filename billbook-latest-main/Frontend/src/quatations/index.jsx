...
const Quotations = () => {
  ...
  console.log("deletedQuotations data", deletedQuotations);
  ...
  const handleDeleteQuotation = async (quotationId) => {
    try {
      const response = await axios.delete(
        `http://localhost:8000/api/addQuotation/quotation/delete/${quotationId}`
      );

      if (response.status === 200) {
        console.log("quotation deleted successfully");
        const deletedQuotation = datasource.find(
          (item) => item._id === quotationId
        );
        setDeletedInvoices((prevDeletedQuotations) => [
          ...prevDeletedQuotations,
          deletedQuotation,
        ]);
        setDatasource((prevData) =>
          prevData.filter((item) => item._id !== quotationId)
        );
      } else {
  ...
  const handleDateRangeChange = (dates) => {
    ...
    const filteredData = datasource.filter((record) => {
      const quotationDate = new Date(record.quotationDate);
      return quotationDate >= dates[0] && quotationDate <= dates[1];
    });

    setFilteredDatasource(filteredData);
  };

  const confirmDeleteQuotation = (quotationId) => {
    if (window.confirm("Are you sure you want to delete this invoice?")) {
      handleDeleteQuotation(quotationId);
    }
  };
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8000/api/addQuotation/newDeletedQuotation/quotation")
      .then((response) => {
        console.log("deletedquotation", response.data);
        setDeletedQuotations(response.data);
      })
      .catch((error) => {
  ...
  const deletedQuotationsColumns = [
    {
      title: "Purchase ID",
          ...
          <div className="text-end">
            <div className="dropdown dropdown-action">
              ...
              <div className="dropdown-menu dropdown-menu-end">
                ...
                <button
                  className="dropdown-item"
                  onClick={() => confirmDeleteQuotation(record._id)}
                >
  ...
  const columns = [
    {
      ...
      sorter: (a, b) => a.quotationNumber.length - b.quotationNumber.length,
    },
    {
      title: "Quotation To",
      dataIndex: "customerName",
